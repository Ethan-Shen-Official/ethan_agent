...
The current context builder includes runtime metadata, root AGENTS.md, the active branch history, and tool schemas. Harness now supports lightweight Compact: a compaction record is appended to the active Session Tree, while the next provider request projects a summary plus messages after the cut point. Original records remain available for inspection and rollback. Session display names are append-only `session_info` metadata records and are excluded from model context. Token estimation, advanced turn-prefix handling, CustomMessage projection, and interactive fork commands remain extension points. REPL `/checkout` and `/rollback` only move the session leaf; they do not restore workspace files.

REPL command definitions are centralized in `src/cli/commands.py` so CLI and future TUI can share help, aliases, argument validation, and handlers. `/new` replaces the active session with a new JSONL file; `/name [name]` appends display-name metadata; `/resume [session-id]` either resolves an explicit id or presents a numbered session selector. `/drop <session-id>` requires confirmation and only deletes a non-active managed workspace session; attempting to drop the active session is rejected. `/tree` exposes a read-only structured view of all persisted nodes and marks the active path; `/checkout` continues to resolve stable message IDs. Each JSONL keeps its own `.head` branch pointer; switching sessions replaces the Harness store and restores its active path.

The Harness uses three mutable workspace permission modes: `default`, `accept_edits`, and `bypass_permissions`. CLI startup accepts `--permission-mode`; an interactive REPL can query or change the active policy with `/permission_mode`, `/permission_mode <mode>`, or the short form `/perm [d|e|b]` (`d=default`, `e=accept_edits`, `b=bypass_permissions`). The change affects subsequent tool calls without restarting the Loop or changing the session transcript. Protected `.agent` and `.git` paths remain blocked in every mode.

The workspace permission policy also hard-denies common workspace-wide destructive shell commands (`rm -rf .`, `rmdir /s /q .`, `Remove-Item -Recurse -Force .`, `find . -delete`, `git clean -f`, and equivalent root/wildcard forms) before the shell starts, including in `bypass_permissions` mode.
